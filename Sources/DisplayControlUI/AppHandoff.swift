...
public enum AppHandoff {
    public static func openPresetManager() {
        ...
        // 2. Search adjacent directories (e.g. .build or /Applications)
        let possibleUrls = [
            Bundle.main.bundleURL.deletingLastPathComponent().appendingPathComponent("DisplayControl.app"),
            URL(fileURLWithPath: "/Users/spar/xcode-local/remote-screen/.build/DisplayControl.app"),
            URL(fileURLWithPath: "/Applications/DisplayControl.app")
        ]

        for url in possibleUrls where FileManager.default.fileExists(atPath: url.path) {
            NSWorkspace.shared.openApplication(at: url, configuration: NSWorkspace.OpenConfiguration())
            return
        }

        // 3. Fallback: activate directly if WindowManager is available in-process
        WindowManager.shared.showPresetManager(store: PresetStore())
    }
}
